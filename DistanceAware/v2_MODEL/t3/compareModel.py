...
class ModelLoader:
    ...
    def load_models(self) -> Tuple[object, object]:
        ...
        # Load Distance-Aware Chronos (trained weights in base_model subfolder)
        print(f"1. Distance-Aware Chronos from {self.config.distance_aware_repo}...")
        self.da_model = ChronosPipeline.from_pretrained(
        ...
        print(f"2. Original Chronos from {self.config.original_repo}...")
...
class BenchmarkEvaluator:
    ...
    def evaluate(self, test_data: List[Dict]) -> Dict:
        ...
        for item in tqdm(test_data, desc="Forecasting"):
            ...
            da_pred = self._forecast(self.da_model, context, horizon)
            orig_pred = self._forecast(self.orig_model, context, horizon)
    ...
    def _forecast(self, model, context: np.ndarray, horizon: int) -> np.ndarray:
...
class ResultsAnalyzer:
    ...
    def generate_report(self, results: Dict) -> pd.DataFrame:
        ...
        self._analyze_per_dataset(results)

        return df

    def _analyze_per_dataset(self, results: Dict):
        """Analyze results per dataset to find where DA performs better"""
        print(f"\n{'='*60}\nPer-Dataset Analysis\n{'='*60}")

        # Build combined dataframe
        records = []
        for i, (da, orig, meta) in enumerate(zip(results['da'], results['orig'], results['meta'])):
            if np.isnan(da.get('MAE', np.nan)) or np.isnan(orig.get('MAE', np.nan)):
                continue
            records.append({
                ...
                'da_rmse': da['RMSE'],
                'orig_rmse': orig['RMSE'],
                'da_wins': 1 if da['MAE'] < orig['MAE'] else 0
            })

        if not records:
            print("No valid records for per-dataset analysis")
            return
        ...
        dataset_stats.columns = ['DA_MAE', 'Orig_MAE', 'DA_RMSE', 'Orig_RMSE', 'DA_Wins', 'Total']
        dataset_stats['Win_Rate_%'] = (dataset_stats['DA_Wins'] / dataset_stats['Total'] * 100).round(1)
        dataset_stats['MAE_Improvement_%'] = ((dataset_stats['Orig_MAE'] - dataset_stats['DA_MAE']) / dataset_stats['Orig_MAE'] * 100).round(2)
        dataset_stats['Winner'] = dataset_stats['MAE_Improvement_%'].apply(lambda x: '🏆 DA' if x > 0 else 'Original')

        # Sort by DA win rate
        dataset_stats = dataset_stats.sort_values('Win_Rate_%', ascending=False)

        print("\n" + dataset_stats.to_string())

        # Save to CSV
        dataset_stats.to_csv(self.output_dir / "per_dataset_results.csv")
        print(f"\n✓ Saved: {self.output_dir / 'per_dataset_results.csv'}")

        # Highlight best datasets for DA
        best_for_da = dataset_stats[dataset_stats['Win_Rate_%'] > 50]
        if not best_for_da.empty:
            print(f"\n{'='*60}")
            print("📊 DATASETS WHERE DISTANCE-AWARE PERFORMS BETTER:")
            print(f"{'='*60}")
            for ds in best_for_da.index:
                row = best_for_da.loc[ds]
                print(f"  • {ds}: {row['Win_Rate_%']}% win rate, {row['MAE_Improvement_%']:+.2f}% MAE improvement")
